...
public class MemberFrame extends JFrame implements ActionListener,PropertyChangeListener {
	...
	public MemberFrame() {
		...
		model=new DefaultTableModel(colNames, 0) { 
			//인자로 전달되는 행(row), 행(column) 수정 가능 여부를 리턴하는 메소드
			@Override
...
//		model.addRow(row2);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(command.equals("save")) {
			...
			if (isSuccess) {
				JOptionPane.showMessageDialog(this, name+" 님의 정보가 추가 되었습니다.");
			}else {
				JOptionPane.showMessageDialog(this, name+" 추가 실패.");
			}
			//JTable에 목록 다시 출력하기
			displayMember();
			...
			if(selectedIndex==-1) {//선택된 row가 없으면.
				return;//메소드를 여기서 끝내라.(리턴해라)
			}
			//실제 삭제 할 것 인지 확인을 한다.
			int selection=JOptionPane.showConfirmDialog(this, "선택된 row를 삭제 하겠습니까?");
			if(selection != JOptionPane.YES_OPTION) {
				return;
			}
			
			//2. 선택된 row의 첫번째 칼럼의 숫자를 읽어온다.(삭제 할 회원의 번호)
