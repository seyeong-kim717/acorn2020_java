...
public class Quiz03 extends JFrame implements ActionListener {
	...
	//생성자
	public Quiz03() {
		//프레임의 제목 설정(JFrame 상속값)
		setTitle("나의 파일");
		//프레임의 레이아웃 지정(JFrame 상속값)
		setLayout(new BorderLayout());
		//메뉴 아이템 3개 만들기
		JMenuItem item_new=new JMenuItem("New");
		JMenuItem item_open=new JMenuItem("Open");
		//default 생성자를 호출해서 객체 생성한후
		JMenuItem item_save=new JMenuItem();
		//아이템 text를 메소드를 이용해서 전달도 가능하다.
	...
	@Override
	public void actionPerformed(ActionEvent e) {//e는 이벤트
			//눌러진 아이템의 액션 command를 읽어 온다.
		String command=e.getActionCommand();
		if(command.equals("new")) {//아이템 new를 눌렀을때
			area.setText("");//문자열 삭제
			area.setVisible(true);//JTextArea를 보이게 하고
			area.grabFocus();//포커스를 준다. 커서가 깜박 거리는 효과.
		}else if(command.equals("open")) {//아이템 open을 눌렀을때
			area.setText("");
			openContent();
		}else if(command.equals("save")) {//아이템 save를 눌렀을때
			saveContent();
		}
	}
	...
	public void saveContent() {
		...
		if(result == JFileChooser.APPROVE_OPTION) {
			...
			try {
				FileWriter fw=new FileWriter(file);//파일 출력시 
				fw.write(content);
				fw.flush();
				fw.close();
				JOptionPane.showMessageDialog(this, file.getName()+"저장 했습니다");
			} catch (IOException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
